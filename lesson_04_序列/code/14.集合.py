集合：存储不可变对象，而且无序，不能出现重复元素
使用 {} 来创建集合，复习：序列用[],元组用(),字典用{}
s = {10,3,5,1,2,1,2,3,1,1,1,1} # <class 'set'>
# s = {[1,2,3],[4,6,7]} TypeError: unhashable type: 'list'
# 使用 set() 函数来创建集合，如果直接s={}则是字典
s = set() # 空集合
# 可以通过set()来将序列和字典转换为集合
s = set([1,2,3,4,5,1,1,2,3,4,5])  #序列和元组都可以
s = set('hello')                  #字符串也可以，因为字符串也是序列
s = set({'a':1,'b':2,'c':3}) # 使用set()将字典转换为集合时，只会包含字典中的键

# 创建集合
s = {'a' , 'b' , 1 , 2 , 3 , 1}
无法通过索引来取得集合中的元素，只能通过list()转为序列后才可以通过索引取值
